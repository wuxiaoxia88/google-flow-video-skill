...
import { FlowBridgeError, createModelPolicy, type GenerationMode, type ModelPolicy, type ProviderModelFamily, type RuntimeSubmissionSnapshot, type VideoGenerationRequest } from "../../contracts/src/index.js";
...
const modeLabels: Record<GenerationMode, string> = { text_to_video: "Text to video", extend_video: "Extend video", first_frame_to_video: "First frame to video", first_last_frames_to_video: "First and last frames to video", ingredients_to_video: "Ingredients to video",edit_video:"Edit video" };
...
export class SemanticFlowPage {
  ...
  async readSnapshot(request: VideoGenerationRequest): Promise<RuntimeSubmissionSnapshot> {
    ...
    const modelOption = await this.readSelectedOption(labels.model, "model");
    ...
    const capturedAt=new Date().toISOString(), capture=await this.captureCapabilities(capturedAt);
    return { configurationVerified: true, visibleAccountContext: null, projectRef, model, modelOption, observedProviderFamily:providerFamilyFromVisibleModel(modelOption.label,modelOption.value), capabilityCapture:{hash:capture.hash,capturedAt:capture.capturedAt,locale:capture.locale,source:"page_visible"}, mode, durationSeconds, aspectRatio, resolution, outputs, totalCredits, availableCredits: balance.balance, capturedAt, capabilityFingerprint: capture.hash };
  }
  ...
  /** Read-only visible options plus the current combination; no controls are changed to infer a matrix. */
  async readObservedCombination(): Promise<{ observed: { model: string; modelOption:{label:string;value:string}; modelOptions:Array<{label:string;value:string;family:ProviderModelFamily}>; capabilityHash:string; locale:string|null; mode: GenerationMode; aspectRatio: string; durationSeconds: number; resolution: string; outputs: number } | null; source: "page_visible" | "unknown"; capturedAt: string; reason?: string }> {
    try {
      const capturedAt=new Date().toISOString(),capture=await this.captureCapabilities(capturedAt),modelOption=await this.readSelectedOption(labels.model,"model"); const mode = parseMode(await this.readControl(labels.mode, "generation mode"));
      ...
      return { observed: { model:modelOption.value,modelOption,modelOptions:capture.modelOptions,capabilityHash:capture.hash,locale:capture.locale, mode, aspectRatio, durationSeconds, resolution, outputs }, source: "page_visible", capturedAt };
  ...
  async configure(request: VideoGenerationRequest): Promise<void> {
    ...
    const modelOption=await this.resolveModelOption(request.modelPolicy??createModelPolicy(request.model,"explicit"),request.model);
    await this.choose(labels.mode, modeLabels[request.mode]); await this.choose(labels.model, modelOption.value); await this.choose(labels.ratio, request.aspectRatio);
  ...
  async visibleAssets(): Promise<Array<{ ref: string; status: "generating" | "generated" | "failed"; modelReadback:{family:ProviderModelFamily;label:string|null;value:string|null;source:"result_card"|"unknown"} }>> {
    ...
    const found: Array<{ ref: string; status: "generating" | "generated" | "failed"; modelReadback:{family:ProviderModelFamily;label:string|null;value:string|null;source:"result_card"|"unknown"} }> = [];
    for (let index = 0; index < count; index++) {
      ...
      const modelLabel=(await card.getAttribute?.("data-model-label"))?.trim()||null,modelValue=(await card.getAttribute?.("data-model-value"))?.trim()||null;
      const family=modelLabel&&modelValue?providerFamilyFromVisibleModel(modelLabel,modelValue):"unknown";
      found.push({ ref, status: raw, modelReadback:{family,label:modelLabel,value:modelValue,source:family==="unknown"?"unknown":"result_card"} });
    }
    return found;
  }
  ...
  private async optionPairs(label:RegExp,what:string):Promise<Array<{label:string;value:string}>>{
    const control=await this.one(this.page.getByLabel(label),what),options=control.locator?.("option");
    if(!options||!options.nth)throw new FlowBridgeError("UNSUPPORTED_COMBINATION",`Flow UI cannot enumerate visible ${what} options.`);
    const count=await options.count(),found:Array<{label:string;value:string}>=[];
    for(let i=0;i<count;i++){const option=options.nth(i),value=await option.getAttribute?.("value"),labelText=(await option.textContent?.())?.trim();if(!value||!labelText)throw new FlowBridgeError("UNSUPPORTED_COMBINATION",`Flow UI exposed an ambiguous ${what} option.`);found.push({label:labelText,value});}
    if(!found.length||new Set(found.map(x=>x.value)).size!==found.length)throw new FlowBridgeError("UNSUPPORTED_COMBINATION",`Flow UI exposed no unique ${what} options.`);
    return found;
  }
  private async readSelectedOption(label:RegExp,what:string):Promise<{label:string;value:string}>{const control=await this.one(this.page.getByLabel(label),what),value=(await control.inputValue?.())?.trim();if(!value)throw new FlowBridgeError("UNSUPPORTED_COMBINATION",`Flow UI did not expose selected ${what} value.`);const pairs=await this.optionPairs(label,what),selected=pairs.find(x=>x.value===value);if(!selected)throw new FlowBridgeError("UNSUPPORTED_COMBINATION",`Flow UI selected ${what} value has no visible label.`);return selected;}
  private async resolveModelOption(policy:ModelPolicy,requestedModel:string):Promise<{label:string;value:string}>{const options=await this.optionPairs(labels.model,"model");const matches=options.filter(option=>providerFamilyFromVisibleModel(option.label,option.value)===policy.requiredFamily);if(policy.requiredFamily==="veo"){const exact=matches.filter(option=>[option.label,option.value].some(x=>x.toLowerCase()===requestedModel.toLowerCase()));if(exact.length===1)return exact[0];throw new FlowBridgeError("UNSUPPORTED_COMBINATION","Explicit Veo request does not exactly match one presently visible Flow UI option",{requestedModel,visibleVeoOptions:matches});}if(matches.length!==1)throw new FlowBridgeError("UNSUPPORTED_COMBINATION","Gemini Omni Flash 1.1 is absent or ambiguous in the current Flow UI; no fallback is allowed",{matchCount:matches.length});return matches[0];}
  private async captureCapabilities(capturedAt:string):Promise<{hash:string;capturedAt:string;locale:string|null;modelOptions:Array<{label:string;value:string;family:ProviderModelFamily}>}>{const modelOptions=(await this.optionPairs(labels.model,"model")).map(x=>({...x,family:providerFamilyFromVisibleModel(x.label,x.value)}));const locale=(await this.page.locator("html").getAttribute?.("lang"))||null;const payload=JSON.stringify({version:1,locale,modelOptions});return{hash:`sha256:${createHash("sha256").update(payload).digest("hex")}`,capturedAt,locale,modelOptions};}
  ...
  private async upload(request: VideoGenerationRequest): Promise<void> {
    const files = [request.inputs?.firstFrame?.path, request.inputs?.lastFrame?.path, ...(request.inputs?.ingredients?.map(x => x.path) ?? []),request.inputs?.sourceVideo?.path].filter((x): x is string => Boolean(x));
...
export function parseVisibleCredits(text: string): number | null {
  ...
  if(/\b(?:each|per\s+(?:output|video|generation|clip)|unit\s+price)\b/i.test(clean))return null;
...
function parseMode(value: string): GenerationMode {
  ...
  if (compact === "text_to_video" || compact === "extend_video" || compact === "first_frame_to_video" || compact === "first_last_frames_to_video" || compact === "ingredients_to_video" || compact==="edit_video") return compact;
