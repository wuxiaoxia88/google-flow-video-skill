...
import { FLOW_CREDIT_HARD_CAP, FlowBridgeError, type FlowProvider, type PreparedSubmission, type ProviderAsset, type ProviderContext, type RuntimeSubmissionSnapshot, type VideoGenerationRequest } from "../../contracts/src/index.js";
...
export class MockProvider implements FlowProvider {
  ...
  async submitPrepared(request:VideoGenerationRequest,prepared:PreparedSubmission,context:ProviderContext){
    ...
    if(readback>request.costPolicy.maxCredits)throw new FlowBridgeError("COST_LIMIT_EXCEEDED","Mock final readback exceeds whole-job cap",{totalCredits:readback,authorizedCeiling:request.costPolicy.maxCredits});
    this.submitCount++;
