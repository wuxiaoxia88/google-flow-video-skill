...
import { assertFreshExistingBrowserObservation, hasStableAccountFingerprint } from "../../flow-adapter/src/index.js";
...
export class JobEngine {
  ...
  completeExistingBrowser(ticketId:string,evidence:ExistingBrowserCompletionEvidence):JobRecord {
    ...
    const actual=evidence.actual,expected=[request.model,request.mode,request.aspectRatio,request.durationSeconds,request.resolution,request.outputs],observed=[actual.model,actual.mode,actual.aspectRatio,actual.durationSeconds,actual.resolution,actual.outputs];
...
function observationSnapshot(o:ExistingBrowserObservation):RuntimeSubmissionSnapshot{if(!o.configuration)throw new FlowBridgeError("UNSUPPORTED_COMBINATION","Generate tickets require a complete current-page configuration observation");return {configurationVerified:true,visibleAccountContext:o.visibleAccountContext,projectRef:o.projectRef,model:o.configuration.model,mode:o.configuration.mode,durationSeconds:o.configuration.durationSeconds,aspectRatio:o.configuration.aspectRatio,resolution:o.configuration.resolution,outputs:o.configuration.outputs,totalCredits:o.totalCredits,availableCredits:o.availableCredits,capturedAt:o.observedAt,capabilityFingerprint:`existing-browser:${o.browserId}:${o.tabId}`,selectedSourceAssetRef:o.selectedSourceAssetRef??null,baselineAssetRefs:o.visibleAssetRefs??[]};}
